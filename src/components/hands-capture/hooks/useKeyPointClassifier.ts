...
const calcLandmarkList = (image, landmarks) => {
  ...
  // Keypoint
  Object.values(landmarks).forEach((landmark: unknown) => {
    const { x, y } = landmark as Landmark;
    const landmarkX = Math.min(x * imageWidth, imageWidth - 1);
    const landmarkY = Math.min(y * imageHeight, imageHeight - 1);
